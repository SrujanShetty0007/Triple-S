...
const SEMESTER_SUBJECTS = {
    sem1: ['chemistry', 'electronics', 'embedded-systems', 'engineering-drawing', 'english', 'health', 'kannada', 'mathematics'],
    sem2: ['caed', 'chemistry', 'electrical', 'mathematics', 'physics', 'programming-c', 'python'],
    sem3: ['data-structures'],
    sem4: ['algorithms', 'algorithms-lab', 'biology', 'database-systems', 'discrete-mathematics', 'human-values', 'microcontrollers', 'mongodb-lab'],
    sem5: ['database-systems'],
    sem6: ['web-technologies'],
    sem7: ['machine-learning'],
    sem8: ['cloud-computing']
};
...
function generateManifest() {
    ...
    semesterDirs.forEach(semester => {
        ...
        const subjectDirs = SEMESTER_SUBJECTS[semester] ||
            fs.readdirSync(semesterPath).filter(item => fs.statSync(path.join(semesterPath, item)).isDirectory());
        ...
        subjectDirs.forEach(subject => {
            ...
            expectedMaterialTypes.forEach(materialType => {
                ...
                const pdfFiles = fs.existsSync(materialPath) ? scanDirectory(materialPath) : [];
